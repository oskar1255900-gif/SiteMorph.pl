  ...
  return (
    ...
    >
      <motion.div variants={cineSoft} className="flex items-center gap-4 mb-10">
        ...
        <div>
          <h1 className="sm-h1">Poradniki</h1>
      ...
      <motion.div variants={cineParent} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-2 gap-6" style={{ perspective: 1600 }}>
        {ACADEMY_GUIDES.map((g, i) => (
          <motion.div
            ...
            className="rounded-[14px] p-6 transition-all flex flex-col justify-between cursor-pointer hover:shadow-lg" style={{ background: 'var(--sm-surface)', transformStyle: 'preserve-3d' }}
          >
            <div>
              <div className="flex justify-between items-center mb-3">
                <span className="text-[13px] font-medium px-2.5 py-1 rounded-[8px]" style={{ background: 'var(--sm-surface-hover)' }}>{g.category}</span>
              ...
              <p className="text-[14px] leading-relaxed" style={{ color: 'var(--sm-text-secondary)' }}>{g.excerpt}</p>
            </div>
            <div className="flex items-center justify-between pt-4 mt-4" style={{ borderTop: '1px solid var(--sm-border-subtle)' }}>
              <span className="text-[14px] font-medium" style={{ color: 'var(--sm-text-secondary)' }}>Poziom: {g.level}</span>
        ...
        {activeGuide !== null && (
          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-2xl max-h-[86vh] overflow-hidden rounded-[16px] shadow-2xl flex flex-col" style={{ background: 'var(--sm-bg)', perspective: 1200 }}
            >
              
              <div className="relative flex items-center justify-between p-6 backdrop-blur sticky top-0" style={{ borderBottom: '1px solid var(--sm-border-subtle)', background: 'var(--sm-bg)' }}>
                <div className="pr-4">
                  <div className="text-[13px] font-medium px-2.5 py-1 rounded-[8px] w-fit" style={{ background: 'var(--sm-surface-hover)' }}>{ACADEMY_GUIDES[activeGuide].category} - {ACADEMY_GUIDES[activeGuide].time}</div>
                ...
                <motion.button whileHover={{ scale: 1.08, rotate: 90 }} whileTap={{ scale: 0.92 }} onClick={() => setActiveGuide(null)} className="w-8 h-8 rounded-full grid place-items-center cursor-pointer shrink-0" style={{ background: 'var(--sm-surface-hover)' }}>
                  <X size={14} />
                </motion.button>
              </div>
              <div className="relative overflow-y-auto p-6 space-y-6 no-scrollbar text-left" style={{ fontFamily: "'Inter', sans-serif" }}>
                ...
                <div className="pt-4 flex justify-end" style={{ borderTop: '1px solid var(--sm-border-subtle)' }}>
