  ...
  return (
    ...
    >
      <motion.div variants={cineSoft} className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-semibold" style={{ fontFamily: "'Inter', sans-serif" }}>Własne domeny</h1>
          <p className="text-[13px] font-bold opacity-80">Podłącz domenę swojej firmy i publikuj strony bez linku demo.</p>
        </div>
      </motion.div>
      ...
      {needLogin ? (
        <motion.div variants={itemVariants} className="rounded-2xl border p-6 bg-[#F7F6F3]/40 dark:bg-neutral-950 border-[#EAEAEA] dark:border-neutral-800 text-center space-y-2">
          <p className="text-sm font-semibold">Zaloguj się, aby zarządzać domenami</p>
          <p className="text-[13px] font-bold opacity-70">Domeny są przypisane do Twojego konta SiteMorph.</p>
        </motion.div>
      ) : loading ? (
        ...
        <>
          ...
          <motion.div variants={itemVariants} className="rounded-2xl border p-5 mb-6 bg-white dark:bg-neutral-950 border-[#EAEAEA] dark:border-neutral-800">
            <h3 className="text-sm font-semibold mb-3">Podłącz domenę</h3>
            {!planOk ? (
              <div className="rounded-xl border border-amber-200 dark:border-amber-900/40 bg-amber-50/60 dark:bg-amber-950/20 p-4 text-[13px] font-bold space-y-1">
                <p className="font-semibold">Własne domeny są dostępne w pakietach od 100 zł/mies (Pro, Business, Agencja).</p>
                <p className="opacity-70">Twój obecny plan: <span className="font-semibold uppercase">{plan}</span>. Ulepsz plan w zakładce „Cennik & Plany”, aby podpiąć domenę swojej firmy.</p>
              </div>
            ) : pages.length === 0 ? (
              <p className="text-[13px] font-bold opacity-70">Najpierw opublikuj stronę w Kreatorze AI - potem wróć tutaj i podepnij pod nią swoją domenę.</p>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_auto] gap-3">
                <select
                  value={pageId}
                  onChange={(e) => setPageId(e.target.value)}
                  className="px-3 py-2 rounded-xl border text-[13px] font-bold outline-none bg-[#F7F6F3]/40 dark:bg-neutral-900 border-[#EAEAEA] dark:border-neutral-800 cursor-pointer max-w-full truncate"
                >
                  {pages.map((p) => (
                ...
                <input
                  ...
                  className="px-3 py-2 rounded-xl border text-[13px] font-bold outline-none bg-[#F7F6F3]/40 dark:bg-neutral-900 border-[#EAEAEA] dark:border-neutral-800 placeholder:text-blue-400 dark:placeholder:text-neutral-500"
                />
                <Button variant="primary" size="sm" onClick={handleAttach} disabled={attaching || !domainInput.trim()} className="font-semibold whitespace-nowrap">
                  <Link2 size={14} /> {attaching ? 'Podpinam…' : 'Podłącz'}
                </Button>
              </div>
            )}
            {err && <p className="text-[13px] font-semibold text-rose-600 dark:text-rose-400 mt-3">{err}</p>}
          </motion.div>
          ...
          <AnimatePresence>
            {dnsInfo && (
              ...
              >
                <h3 className="text-sm font-semibold mb-1">Krok 2 - ustaw rekordy DNS u rejestratora</h3>
                <p className="text-[13px] font-bold opacity-70 mb-4">
                  Wejdź w panel zarządzania DNS tam, gdzie kupiłeś domenę <span className="font-semibold">{attachedDomain}</span> i dodaj wpisy:
                </p>
                ...
                <p className="text-[13px] font-bold opacity-60 mt-3 flex items-start gap-1.5">
          ...
          <motion.div variants={itemVariants} className="rounded-2xl border bg-white dark:bg-neutral-950 border-[#EAEAEA] dark:border-neutral-800 overflow-hidden">
            <div className="p-4 border-b border-[#EAEAEA] dark:border-neutral-900 flex items-center justify-between">
              <h3 className="text-sm font-semibold">Twoje strony</h3>
              <button onClick={() => load()} className="text-[13px] font-semibold uppercase tracking-widest opacity-60 hover:opacity-100 flex items-center gap-1 cursor-pointer bg-transparent border-none">
                <RefreshCw size={11} /> Odśwież
              </button>
            </div>
            <div className="divide-y divide-blue-50 dark:divide-neutral-900">
              {pages.length === 0 ? (
                <div className="p-10 text-center text-[13px] font-bold opacity-60">Brak opublikowanych stron</div>
              ) : (
                pages.map((p) => (
                  <div key={p.id} className="p-4 flex flex-col sm:flex-row sm:items-center gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold truncate">{p.title}</div>
                      <a href={`${apiOrigin}/p/${p.id}`} target="_blank" rel="noreferrer" className="text-[13px] font-bold opacity-60 hover:text-emerald-500 inline-flex items-center gap-1 mt-0.5">
                        {apiOrigin}/p/{p.id} <ExternalLink size={10} />
                      </a>
                      {p.custom_domain && (
                        <div className="text-[13px] font-semibold mt-1 flex items-center gap-2">
                          <Globe size={12} />
                          {p.custom_domain}
                          {p.domain_verified
                            ? <span className="inline-flex items-center gap-1 text-[13px] font-semibold text-emerald-600 dark:text-emerald-400"><Check size={11} /> AKTYWNA</span>
                            : <span className="text-[13px] font-semibold text-amber-600 dark:text-amber-400">OCZEKUJE NA DNS</span>}
                        </div>
                      )}
                    </div>
          ...
          {verifyMsg && (
            <p className="text-[13px] font-semibold mt-4 text-[#2563eb] dark:text-white opacity-80">{verifyMsg}</p>
          )}

          <div className="mt-8 rounded-2xl border p-4 bg-[#F7F6F3]/40 dark:bg-neutral-950 border-[#EAEAEA] dark:border-neutral-800 text-[13px] font-bold leading-relaxed opacity-80">
            <span className="font-semibold">Jak to działa?</span> Kupujesz domenę gdziekolwiek (np. OVH, home.pl, Namecheap), wpisujesz u nich dwa rekordy DNS z tabelki powyżej, klikasz „Sprawdź teraz” - i gotowe. Strona działa na Twojej domenie, hosting pozostaje po naszej stronie.
          </div>
        </>
      )}
    </motion.div>
