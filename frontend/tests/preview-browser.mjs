...
try {
  ...
  await ui.getByText(/Brakuje kredytów/).waitFor();
  assert.equal(calls, 0);
  await ui.evaluate(() => window.__setCredits(500));
  await ui.locator('textarea').press('Enter');
  await ui.locator('[data-testid="preview-status"]').filter({ hasText: 'Podgląd gotowy' }).waitFor({ timeout: 30000 });
  ...
  await ui.getByRole('button', { name: /Zbuduj nową stronę/ }).click();
